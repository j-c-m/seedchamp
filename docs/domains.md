...
| **CLI** | `serve`, `torrent …`, `import rtorrent`, `watch`, `doctor`, `config`, `bench`, `version` | Non-interactive; `torrent start|stop|recheck|del` are catalog-only; swarm via `serve` / TUI |
